<script lang="ts">
	...
	let page: Page;
	pageStore.subscribe((v) => (page = v));

	const splitURL = (url: string): [string, string] => {
		if (page.params.type == 'gh') {
	...
	let [p1, p2] = splitURL(page.params.url);
</script>
...
>
	...
	<div class="text-3xl font-bold max-w-4xl text-center break-words my-5">
		...
		<span class="opacity-50">from “https://denopkg.dev/{type}/</span>{p1}<span class="opacity-50"
			>{p2}”</span
		>
	</div>
	<button
		on:click={() => {
			copyToClipboard(`import * from "https://denopkg.dev/${type}/${page.params.url}"`);
		}}
