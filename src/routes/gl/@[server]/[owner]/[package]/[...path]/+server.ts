...
const getDefaultBranch = async (server: string, owner: string, repo: string, headers: Headers) => {
	const respData = await (
		await fetch(`https://${server}/${owner}/${repo}/-/branches`, {
			headers: {
				'user-agent':
...
const handler: RequestHandler = async (req) => {
	...
	const resp = await fetch(
		`https://${params.server}/${params.owner}/${params.package}/-/raw/${params.tag}/${params.path}`,
		{
			headers: {
