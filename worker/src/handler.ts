export const run = async (req: Request) => {
  ...
  switch (type) {
    ...
    case "gh": {
      ...
      // check for version
      if (data.version == "") {
        const latestTag = (await (await fetch(
    ...
    case "gl": {
      ...
      if (!data.version) {
        const defaultBranch = (await (await fetch(
          `https://gitlab.com/${data.owner}/${data.repo}/-/branches`,
          req,
        ))
          .text()).match(/data-default-branch="([^"]+)"/);
...
const redirectPath = (path: string, reqUrl: string) => {
  ...
  return send.redirect(
    new URL("./mod.ts", reqUrl + (reqUrl.slice(-1) != "/" ? "/" : ""))
      .toString(),
  );
};
...
const send = {
  ...
  website: async (req: Request) => {
    ...
    let resp = await fetch(
      "https://deno.pages.dev" + url.pathname + url.search,
      req,
    );
